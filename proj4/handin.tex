...
From this can be seen that ImageLoad.dll is not relocatable and ASLR is not enabled, and that fmws.exe is relocatable but ASLR is not enabled. Now, with a hypothesis about our experiment (i. e. memory location for fmws.exe contra Imageload.dll and stack addresses will not change between debugging sessions or VM restarts) from above, let us register stack memory address for ImageLoad.dll and fmws.exe between to debugging sessions. We set a breakpoint at \texttt{kungfu} gadget's address and also check the memory address when fmws.exe is entered.

Run 1
\begin{center}
  \begin{tabular}{ | l | c | r }
    \hline
    Memory address IL &  1 & 2  \\ \hline
    Stack address IL & 5 & 6  \\ \hline
    Memory  address IL & 5 & 6  \\ \hline
    Memory address fmws &  1 & 2  \\ \hline
    Stack address fmws & 5 & 6  \\ \hline
    Memory  address fmws & 5 & 6  \\ \hline
  \end{tabular}
\end{center}
